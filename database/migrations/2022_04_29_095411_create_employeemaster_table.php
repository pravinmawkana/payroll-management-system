...
class CreateEmployeemasterTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('employeemaster', function (Blueprint $table) {
            $table->id('empId');
            $table->string('empCode',15);
            $table->string('empTitle',10)->nullable();
            ...
            $table->foreign('eBankId')->references('eBankId')->on('employeebankmaster');
            ...
            $table->string('reginRemarks')->nullable();
            $table->boolean('isSalaryProcessing');
            $table->string('pfUNANo',50);
            $table->string('attendanceCode',50);
