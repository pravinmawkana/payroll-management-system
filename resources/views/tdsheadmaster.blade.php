...
<form name="frmTDSHeadMaster" id="frmTDSHeadMaster" method="post"  action="TDSHeadMaster">
    @csrf
  <div class="form-row">
    <div class="form-group col-md-6">
      <h3>TDS Head Master</h3>
    </div>
    <div class="form-group col-md-6">
  ...
  <div class="form-row">
    <div class="form-group col-md-3">
      <label for="tdsHeadName">TDS Head</label>
      <input type="text" size="50" maxlength="50" class="form-control" id="tdsHeadName" name="tdsHeadName" placeholder="Enter TDS Head" required>
    </div>
     <div class="form-group col-md-6">
      <label for="tdsHeadDesc">TDS Description</label>
      <input type="text" size="255" maxlength="255" class="form-control" id="tdsHeadDesc" name="tdsHeadDesc" placeholder="Enter TDS Description" required>
    </div>
...
$(document).ready(function(){
    ...
    function displayRecords(){
        $.ajax({
            url:'{{route('tdsheadmasterdisplay')  }}',
            method :'get',
    ...
    $('#frmTDSHeadMaster').submit(function(e){
        e.preventDefault();
    ...
    function editData(status,key,tdsHeadId){
        $('.message').html('Click Update button after changes');
        //console.log(tdsHeadId);
        $.ajaxSetup({
    headers: {
        ...
        $.ajax({
            ...
            success:function(response){
                ...
                $('#btnSave').text('Update');
                if(status==1){
                    $('#rowId').val(response.tdsHeadId);
                    $('#tdsHeadName').val(response.tdsHeadName);
                    $('#tdsHeadDesc').val(response.tdsHeadDesc);
                   // $('#cTypeId').val(response.cTypeId);
                    ...
                    //alert('view');
                     $('#viewRecordLabel').html("Branch Code:-"+ response.tdsHeadName);
              $('#viewRecord').modal('show');
              var view = "Division Id :" + response.tdsHeadId  + "<br> TDS Name :-" + response.tdsHeadName +"<br> TDS Description :-" + response.tdsHeadDesc ;
    ...
    function deleteData(status,key,tdsHeadId){
        Swal.fire({
            title: 'Are you sure?',
