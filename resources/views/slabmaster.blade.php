...
<form name="frmSlabMaster" id="frmSlabMaster" method="post"  action="companyslabmaster">
    @csrf
  <div class="form-row">
  ...
  <div class="form-row">
    <div class="form-group col-md-3">
      ...
      <input type="text" size="20" maxlength="20" class="form-control" id="slabName" name="slabName" placeholder="Enter Branch Code" required>
    </div>
...
$(document).ready(function(){
    ...
    $('#frmSlabMaster').submit(function(e){
        e.preventDefault();
    ...
    function editData(status,key,slabId){
    ...
    headers: {
        ...
        $.ajax({
            ...
            success:function(response){
                ...
                $('#btnSave').text('Update');
                if(status==1){
                    $('#rowId').val(response.slabId);
                    $('#slabName').val(response.slabName);
                    ...
                    //alert('view');
                     $('#viewRecordLabel').html("Branch Code:-"+ response.slabName);
              $('#viewRecord').modal('show');
              var view = "Slab Id :" + response.slabId  + "<br> Slab Name :-" + response.slabName ;
    ...
    function deleteData(status,key,slabId){
        Swal.fire({
            title: 'Are you sure?',
