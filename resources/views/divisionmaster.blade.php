...
<form name="frmDivisionMaster" id="frmDivisionMaster" method="post"  action="Divisionmaster">
    @csrf
  <div class="form-row">
  ...
  <div class="form-row">
    <div class="form-group col-md-3">
      ...
      <input type="text" size="50" maxlength="50" class="form-control" id="divName" name="divName" placeholder="Enter Division Name" required>
    </div>
...
$(document).ready(function(){
    ...
    function displayRecords(){
        $.ajax({
            url:'{{route('divisionmasterdisplay')  }}',
            method :'get',
    ...
    $('#frmDivisionMaster').submit(function(e){
        e.preventDefault();
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $('.message').html("Record Update");
                        $.ajax({
                        url: '{{ route('divisionmasterupdate') }}',
                        method : 'post',
                        data: fd,
                        processData: false,
                        contentType: false,
    ...
    function editData(status,key,divId){
    ...
    headers: {
        ...
        $.ajax({
            ...
            success:function(response){
                ...
                $('#btnSave').text('Update');
                if(status==1){
                    $('#rowId').val(response.divId);
                    $('#divName').val(response.divName);
                    ...
                    //alert('view');
                     $('#viewRecordLabel').html("Branch Code:-"+ response.divName);
              $('#viewRecord').modal('show');
              var view = "Division Id :" + response.divId  + "<br> Division Name :-" + response.divName ;
    ...
    function deleteData(status,key,divId){
        Swal.fire({
            title: 'Are you sure?',
            ...
            headers: {
                ...
                $.ajax({
                    url:'{{route('divisionmasterdelete')  }}',
                    method :'post',
                    data: {
                        divId: divId
                    },
